...
% 
% Correlation matrix CorrelSig2Respsec (let's call it just C)is calculated as
% follows.
% 1. Ci,j = corrcoeff(Usi, Yji)
% where 
% a) Usi is an N-elements subvector of RawUnitReg starting at location i
% b) Yji is an N-elements subvector of shifted up by j elements UnitResp 
% starting at location i
% 
% C(i,j) = corrcoeff(RawUnitReg[i:N+i], UnitResp[i-j:N+i-j])
...
for j=1:M,
    ...
    temp = Vshift(UnitResp,S); % new response vector shifted up by S positions.
                               % or shifted in time by S positions (10 sec 
                               % multiple.)
    for i = 1:H1int,
        %The moving window of N+1 elements starting at location i-1 
        % is siplified to move the entire
        % vectors up by i-1 elements and then taking the first N+1
        % elements.
        temp1 = Vshift(RawUnitReg,i-1);% command vector shifted up by i-1 
                               % positions or shifted in time by i-1 positions
                               
        temp2 = Vshift(temp,i-1); %shifting up by the same i-1 positions
                                  %already shifted up response vector

        %Taking only N+1 elements of both vectors to calculate correlation
        %coefficient.
...
title('Correlation Matrix of 5 minute intervals between the command and 10 sec backward shifted response');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% [BJ]
% calculate Fleet Regulation Control Signal.
% It is a PU signal
% Derived RegSignalPercent isl limited to <-1;1>
...
for i = 1:H1int,
    temp1 = Vshift(UnitAREG,i-1);
    if min(temp1(1:N+1))==0,
        DelayScoreAdj(i) = NaN;
